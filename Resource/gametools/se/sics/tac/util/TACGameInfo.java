...
public class TACGameInfo {
  ...
  private static final Logger log =
    Logger.getLogger(TACGameInfo.class.getName());
...
//     log.finest("RECEIVED: " + tokenizer.getMessage());
	...
	if (index >= 0) {
      ...
      break;
    case ISTokenizer.BID:
      if (!ignoreBids) {
	int bidID = tokenizer.nextInt();
	int agentID = tokenizer.nextAgentID();
	int agentIndex = getAgentIndex(agentID);
	int auctionID = tokenizer.nextInt();
	int auctionIndex = getAuctionIndex(auctionID);
	if (agentIndex >= 0 && auctionIndex >= 0) {
	  String bidType = tokenizer.nextToken();
	  int processStatus = tokenizer.nextInt();
	  Bid bid = new Bid(bidID, agentIndex, auctionIndex,
			    bidType, processStatus);
	  while (tokenizer.hasMoreTokens()) {
	    bid.addBidPoint(tokenizer.nextInt(),
			    tokenizer.nextFloat());
	  }
	  if (bids == null) {
	    bids = new Bid[500];
	  } else if (bids.length == numberOfBids) {
	    bids = (Bid[]) ArrayUtils.setSize(bids, numberOfBids + 50);
	  }
...
// 	  } catch (Exception e) {
// 	    System.err.println("could not parse score: '"
// 			       + tokenizer.getMessage() + '\'');
// 	    e.printStackTrace();
// 	  }
...
//     int quantity = usedGoods[agentIndex][day][type];
  ...
  /** Bid information (might return null) */
  public Bid[] getBids() {
    if (bids == null) {
      bids = new Bid[0];
    } else if (numberOfBids != bids.length) {
      bids = (Bid[]) ArrayUtils.setSize(bids, numberOfBids);
    }
    return bids;
  }
